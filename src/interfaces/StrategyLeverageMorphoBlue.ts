...
export interface StrategyLeverageMorphoBlueMethods {
  ...
  getPosition(params: {'priceOptions': { 'maxAge': number;'maxConf': number; };}): ContractCallMethod<{'totalCollateralInUSD': number;'totalDebtInUSD': number;'loanToValue': number;}>;

  governor(): ContractCallMethod<string>;

  harvest(): ContractSendMethod;

  initialize(initialOwner: string, initialGovernor: string, registry: string, params: {'params': { 'collateralToken': string;'debtToken': string;'collateralOracle': string;'debtOracle': string;'swapFeeTier': number;'morphoOracle': string;'irm': string;'lltv': number; };}): ContractSendMethod
  ...
  totalAssets(params: {'priceOptions': { 'maxAge': number;'maxConf': number; };}): ContractCallMethod<{'totalOwnedAssetsInDebt': number;}>;
