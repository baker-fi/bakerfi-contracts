...
describeif(network.name === "hardhat")("Settings", function () {
    ...
    it("Change Rebalance Price Max Age ✅", async function () {
        ...
        // @ts-expect-error
        expect(await settings.connect(otherAccount).getRebalancePriceMaxAge()).to.equal(3600);
    });
    ...
    it("Change Price Max Age ✅", async function () {
        ...
        // @ts-expect-error
        expect(await settings.connect(otherAccount).getPriceMaxAge()).to.equal(3600);
    });
    ...
    it("Transfer Settings Ownership in 2 Steps", async function () {
        ...
        expect(await settings.pendingOwner()).to.equal(
            "0x0000000000000000000000000000000000000000"
        );
    ...
    it("Change Price Max Conf ✅", async function () {
        ...
        // @ts-expect-error
        expect(await settings.connect(otherAccount).getPriceMaxConf()).to.equal(10000000n);
    });
...
async function deployFunction() {
    ...
    const {proxy} = await deploySettings(otherAccount.address, serviceRegistry, proxyAdmin);

    const pSettings = await ethers.getContractAt("Settings", await proxy.getAddress());
