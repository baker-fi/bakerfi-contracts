...
describeif(network.name === 'base_devnet')('MilleFeulle - Production', function () {
  it('Vault Settings', async function () {
    ...
    expect(await strategy.getPriceMaxAge()).to.equal(691200);
    expect(await strategy.getPriceMaxConf()).to.equal(0);
  });
...
async function deployFunction() {
  ...
  const contracts = BakerFiContracts[networkName][StrategyImplementation.AAVE_V3_WSTETH_ETH];
  const networkConfig = NetworkConfig[networkName];

  const vault = await ethers.getContractAt('Vault', contracts.vaultProxy);
  const strategy = await ethers.getContractAt('StrategyLeverageAAVEv3', contracts.strategyProxy);
