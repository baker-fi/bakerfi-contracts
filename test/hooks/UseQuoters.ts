...
describeif(network.name === 'hardhat')('UseQuoter', function () {
  ...
  async function deployFunction() {
    ...
    const proxyDeployment = await BakerFiProxy.deploy(
      ...
      UseUniQuoter.interface.encodeFunctionData('initialize', [await serviceRegistry.getAddress()]),
    );
  ...
  it('test singleInput - min Output', async function () {
    ...
    expect(await useUniQuoter.uniQuoterA()).equal(await quoter.getAddress());
