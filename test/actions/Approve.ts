...
import { deployBaseServices, deploySetApproval } from "../common";

describe("Testing Approve Action", function () {
  ...
  it("When i call using a proxy", async function () {
    ...
    await ownerProxy.executeTarget(await setApproval.getAddress(), encodedCall);

    expect(await erc20.allowance(proxyAddress, otherAccount.address)).to.equal(
      ethers.parseUnits("10", 18)
    );
  });
});
