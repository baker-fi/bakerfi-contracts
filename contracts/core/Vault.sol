...
import {WETH_CONTRACT, FLASH_LENDER, SWAP_HANDLER, ST_ETH_CONTRACT} from "./Constants.sol";
...
contract Vault is Ownable, Pausable, ERC20, IERC3156FlashBorrower {
    ...
    string constant SUCCESS_MESSAGE = keccak256("Success");
    
    ServiceRegistry public immutable _registry;
    uint256 private   _ownedCollateral;
    ServiceRegistry public immutable _registry;

    uint256 public immutable constant LOAN_TO_VALUE = 50000;
    ...
    function deposit(address receiver) external payable returns (uint256 shares) {
        ...
        IERC3156FlashLender flashLender = IWETH(_registry.getServiceFromHash(FLASH_LENDER));
        
        uint256 leverage = Leverage.calculateLeverageRatio(msg.value, LOAN_TO_VALUE, 10);
        uint256 loanAmount = leverage - msg.value;
        uint256 fee = flashLoan.flashFee(address(weth), loanAmount);
        ...
        weth.safeApprove(flashLender, loanAmount + fee + allowance );        

        bytes32 result = flashLoan.flashLender(address(this), address(weth), loanAmount, 
            abi.encode(
                msg.value,
                loanAmount,
                receiver
        ...
        require(result == SUCCESS_MESSAGE, "Failed Flash Loan");
    }
    ...
    function onFlashLoan(address initiator, address token, uint256 amount, uint256 fee, bytes data) external returns (bytes32 ) {   
        ...
        FlashLoanData data =  abi.decode(_callData, (FlashLoanData));        
        ...
        uint256 amountOut = swapHandler.executeSwap(
                address(weth),
                stETH,
                0,
                data.originalAmount + data.loanedAmount,
                0
        );

        uint256 amountToPayBack =  data.loanedAmount + fee;

        IPoolV3 aavePool = IPoolV3(_registry.getServiceFromHash(AAVE_V3));   
        aavePool.deposit(stETH, amountOut, address(this), 0);
        aavePool.borrow(weth, amountToPayBack ,  2, 0, address(this));

        IERC3156FlashLender flashLender = IWETH(_registry.getServiceFromHash(FLASH_LENDER));   
        Rebase memory total = Rebase(_ownedCollateral, totalSupply());
        
        shares = total.toBase(amountOut, false);

        _mint(receiver, shares);                 
        
        _ownedCollateral = total.elastic.add(amountOut);

        emit Deposit(initiator, data.receiver, data.originalAmount, data.sharesToMint);
